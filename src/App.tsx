...
export default function App() {
  ...
  const renderMessageContent = (msg: Message) => {
    ...
    if (urlMatch) {
      ...
      if (url.match(/\.(mp4|webm|ogg|ogv|mov|avi|mkv|flv|wmv|m4v|3gp|3g2|mpeg|mpg|mpe|m2v|m4p|qt|swf|avchd|f4v|f4p|f4a|f4b|vob|rm|rmvb|asf|amv|mpg2|mpeg2|ts|m2ts|mts|divx|xvid|dv|mxf)(\?.*)?$/i)) {
        return (
...
{messages.map((msg, idx) => (
  ...
  >
    {msg.username === displayName ? (
      <div
        className="inline-block bg-teal-800/85 text-white p-3 rounded-lg max-w-[85%] backdrop-blur-sm whitespace-pre-wrap break-words"
      ...
      <div
        className="inline-block bg-[#223A56]/95 text-white p-3 rounded-lg max-w-[85%] backdrop-blur-sm whitespace-pre-wrap break-words"
        style={{ marginTop: idx === 0 ? '8px' : '0' }}
      >
      ...
      >
        ...
        <input
          ...
          className="flex-1 bg-white/10 text-white px-3 py-2 rounded-full text-sm outline-none placeholder-gray-400"
        />
        <button
          onClick={sendMessage}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-gray-800 rounded-lg p-6 max-w-sm w-full">
            ...
            <div className="text-gray-300 space-y-2 text-sm">
              <p>🌐 Онлайн чат с общим доступом</p>
              <p>💬 Отправляйте текст, файлы и ссылки</p>
              <p>🎨 Команда <code className="bg-gray-700 px-1">setbg</code> - установить фон</p>
              <p>🎨 Команда <code className="bg-gray-700 px-1">settheme</code> - изменить тему</p>
              <p>👆 Долгое нажатие на сообщение - ответить/удалить</p>
            </div>
          </div>
        </div>
      )}
