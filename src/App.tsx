...
if (url.match(/\.(mp4|webm|ogg|ogv|mov|avi|mkv|flv|wmv|m4v|3gp|mpg|mpeg|ts|m2ts|mts|m3u8|mxf)(\?.*)?$/i)) {
  ...
  const isOnline = (onlineUsers || []).some(u => u.id === msg.senderId) || (onlineUsers || []).some(u => u.username === msg.username)
  return (
  <div
    key={msg.id}
    className="relative"
  ...
  >
    ...
    <div className="w-full flex justify-start items-start">
      <div
        className="inline-block relative text-white p-3 rounded-lg max-w-[85%] whitespace-pre-wrap break-words"
        style={{
      ...
      >
        <div className="absolute -top-2 -left-2 w-4 h-4 rounded-full flex items-center justify-center" style={{ backgroundColor: isOnline ? '#32D74B' : '#6B7280', border: '2px solid white' }} title={isOnline ? 'online' : 'offline'}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="12" height="12" fill="white"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
          </div>
